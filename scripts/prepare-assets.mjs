...
for(const r of refs){
 ...
 for(const selection of selections.filter(c=>c.reference===r.id)){
  ...
  r.crops.push({id:selection.id,path:dest,sheetPath:r.views.find(v=>v.masterImage===im.masterImage).path,purpose:selection.purpose,alt:selection.alt,masterImage:im.masterImage,sourceRect:{x:rect.left,y:rect.top,width:rect.width,height:rect.height},width:output.width,height:output.height,printWidthMm:Math.min(85,Math.floor(output.width*25.4/160)),printHeightMm:Math.min(82,Math.floor(output.height*25.4/160)),transformation:'Extract integer sourceRect; grayscale; 1–99 percentile normalization; fit inside 1800 px, no enlargement; JPEG quality 94',reviewedAt:'2026-09-08'});
 }
}
